<template>
  <div>
    ...
    <v-layout row wrap>
      <v-card style="margin-top: 50px;margin-left:20px;width: 320px;" v-for="product in products" v-bind:key="product._id">
        <h2>{{product.name}}</h2>
        ...
        <div>
          <v-btn @click="deleteProduct(`${product._id}`)" class="left" color="red" >delete</v-btn>
                <v-spacer></v-spacer>
          <v-btn :to="`/admin/edit/${product._id}`"
          class="right" color="primary" >edit</v-btn>
        </div>
      </v-card>
    </v-layout>
  </div>
